...
export function ChatMessages({ messages,handlePreview }: ChatMessagesProps) {
    ...
    <ScrollArea className="flex-1 overflow-y-auto">
      <div className="flex flex-col gap-4 p-6 h-full">
        {messages.length === 0 ? (
          ...
          messages.map((message,index) => <ChatMessage key={`message-${index}`} message={message} handlePreview={handlePreview} />)
        )}
      </div>
    </ScrollArea>
  )
